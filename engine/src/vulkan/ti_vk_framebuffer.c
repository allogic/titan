...
void vk_framebuffer_create(vk_renderpass_t *renderpass, vk_framebuffer_t *framebuffer, char const *asset_path) {
  ...
  while (image_index < image_count) {
    ...
    vk_image_create(&framebuffer->color_attachment[image_index], framebuffer->config->color_attachment_image);
    vk_image_create(&framebuffer->depth_attachment[image_index], framebuffer->config->depth_attachment_image);
    ...
    VkFramebufferCreateInfo frame_buffer_create_info = {
      ...
      .width = framebuffer->config->width ? framebuffer->config->width : g_pl_window.window_width,     // TODO
      .height = framebuffer->config->height ? framebuffer->config->height : g_pl_window.window_height, // TODO
      .layers = 1,
    };
