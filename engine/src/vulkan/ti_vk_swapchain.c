...
void swapchain_create(void) {
  g_swapchain.image_count = clampu(TI_SWAPCHAIN_IMAGE_COUNT, g_window.min_image_count, g_window.max_image_count);
  ...
  TI_VK_CHECK(vkCreateSwapchainKHR(g_window.device, &swapchain_create_info, 0, &g_swapchain.handle));
  TI_VK_CHECK(vkGetSwapchainImagesKHR(g_window.device, g_swapchain.handle, &g_swapchain.image_count, g_swapchain.image));
}
