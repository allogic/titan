...
void fs_framebuffer_load(fs_framebuffer_t *framebuffer, fs_file *file) {
  ...
  fs_file_read(file, &framebuffer->width, sizeof(uint32_t), 0);
  fs_file_read(file, &framebuffer->height, sizeof(uint32_t), 0);
...
void fs_framebuffer_store(fs_framebuffer_t *framebuffer, fs_file *file) {
  ...
  fs_file_write(file, &framebuffer->width, sizeof(uint32_t), 0);
  fs_file_write(file, &framebuffer->height, sizeof(uint32_t), 0);
